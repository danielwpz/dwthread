...
void dwthread_yield()
{
	...
	if (cur_task == NULL) {
		cur_task = malloc(sizeof(dwt_task_t));
		cur_task->tid = current_tid;
...
static void schedule()
{
     ...
     *          and set its status.
	 */
  dwt_task_t* cur_task = dw_find(current_tid);
	if (cur_task->statue == DWT_STATUE_RUNNING)
		cur_task->statue = DWT_STATUE_RUNNABLE;
  ...
  cur_task = dw_next();
	 ...
	 */
	cur_task->statue = DWT_STATUE_RUNNING;
	current_tid = cur_task->tid;
	longjump(&cur_task->context.jmp_restorer,1);
}
